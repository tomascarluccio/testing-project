...
public class MessageDeliveryGatewayPage extends BasePage{
	...
	public MessageDeliveryGatewayPage(WebDriver driver, WebDriverWait wait) {
		...
		this.importLink          	= By.linkText("Import");
	}
	...
	public WebElement getImportLink() {
		return this.driver.findElement(this.importLink);
	}
	
	public boolean existsSMTPGateway(String gtwname) throws Exception{
		String con = API.listMessagingGateways().getContent();
        return con.contains(gtwname);
    }
	
	@Override
	public String getUrl() {
		return "https://" + Config.tnHost + ":" + Config.tnSuperAdminPort.toString() + "/safewalk_gateway/gatewayconfiguration/";
	}
